...
AssignNode::AssignNode(int stmtIndex, std::string varName, std::shared_ptr<ShuntNode> shutNode, std::unordered_set<std::string> exprVariables,
                       std::unordered_set<int> exprConstants)
    : VariableNameNode(stmtIndex, std::move(varName), StmtType::STMT_ASSIGN) ,
      shutNode(std::move(shutNode)),
